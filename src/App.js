...
const OverviewFlow = () => {
  ...
  const ticketsById = tickets.reduce(
    (_, t) => ({ ..._, [t.id]: { ...t, colorid: getRColor() } }),
    {}
  );
  ...
  const show = () => {
    const byId = rfINstance
      .toObject()
      .elements.reduce((_, e) => ({ ..._, [e.id]: e }), {});
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "95vh" }}>
      <div>
        ...
        <button onClick={() => setYaml(null)}>reset</button>
      </div>
      <div style={{ flex: 1 }}>
        <ReactFlow
